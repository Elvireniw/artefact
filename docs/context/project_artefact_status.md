...
>
> **Desktop finalization pass, same day, after mobile scope was set**: site-wide nav anchor links (hero header, footer, mobile menu overlay, plus `.hero__btn`/`.craft__cta`) were fixed/remapped and routed through Lenis instead of native hash-jumps (`initAnchorNav()` in `script.js`) — full mapping and a real pin-arrival-jerk fix in [[project-artefact-nav-anchors]]. A new `.scroll-up` button (mirrors `.hero__scroll`) was added to every anchor-nav target section plus the footer. **Also reviewed 7 pop-up/modal Figma designs** (course-booking ×3 + shared success, free-lesson lead-magnet pair, footer-newsletter success) — assessed as 3 reusable mechanics, not built yet, scoped as its own future session; see [[project-artefact-popup-modals-scope]].
